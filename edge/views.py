...
def create_links(request):
    duplicate_links = []
    with open("edge/static/human_links_01272021.csv") as f:
        reader = csv.reader(f)
        ...
        np.savetxt("edge/static/human_links_01272021_duplicate.csv", duplicate_links_np, delimiter=",", fmt='%s')

    return redirect('edge:select_island')

def create_duplicates(request):
    with open("edge/static/human_links_01272021_duplicate.csv") as f:
        reader = csv.reader(f)
        next(reader, None) # Skip the headers
        for row in reader:
            _, created = ImPair.objects.get_or_create(
